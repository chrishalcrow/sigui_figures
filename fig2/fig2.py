...
if using_real_data:
    ...
    recording, sorting = si.generate_ground_truth_recording(num_channels=96, num_units=96*2, durations=[60*10])
    analyzer = si.create_sorting_analyzer(sorting, recording, peak_sign='both')
    analyzer.compute(['random_spikes', 'noise_levels', 'waveforms', 'templates', 'spike_amplitudes', 'correlograms', 'quality_metrics', 'template_metrics', 'unit_locations'])

print("\nLaunching GUI...\n")

layout = 'fig2/quality_layout.json'
settings_file = 'fig2/quality_settings.json'
